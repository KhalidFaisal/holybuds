...
export default function CartDrawer() {
  const { items, isOpen, setIsOpen, removeItem, updateQuantity, subtotal, discountAmount, discountName, total, addItem, syncCart, syncMessages, setSyncMessages } = useCart();
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-pc-dark border-l border-pc-border z-[70] animate-slide-right flex flex-col">
        ...
        {/* Sync Messages Banner */}
        {syncMessages && syncMessages.length > 0 && (
          <div className="bg-orange-500/10 border-b border-orange-500/20 p-4 relative">
            <button 
            ...
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                {syncMessages.map((msg, i) => (
                  <p key={i} className="text-orange-400 text-sm">{msg}</p>
                ))}
              </div>
            </div>
          </div>
